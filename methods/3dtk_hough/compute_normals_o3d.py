...
writes a uos_normal scan to `--out` (default `<in>/normals_o3d/scan000.3d`).
...
def main():
    ...
    ap.add_argument("--out", dest="outdir", default=None,
                    help="output dir for the uos_normal scan "
                         "(default: <in>/normals_o3d)")
    args = ap.parse_args()
    ...
    out_dir = args.outdir or os.path.join(args.indir, "normals_o3d")
