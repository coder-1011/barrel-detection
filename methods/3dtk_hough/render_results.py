#!/usr/bin/env python3
"""
Render a multi-page PDF showing, for each scene under data/{real,synth}/:
  - the raw point cloud  (front + top 2D projections)
  - the same cloud with the detected cylinder(s) overlaid

Output: <repo>/barrel_detection_results.pdf (one page per scan).

Clouds come from data/{real,synth}/<scene>/scan000.{pcd,3d}; detections come
from methods/3dtk_hough/results/<scene>/. Loads .pcd (m) or .3d (cm), whichever
is present, and normalizes to meters. Cylinder.2d is always in cm. Per scene the
best cylinder.2d is picked in order: normals_o3d/ > normals/ > <scene>/.
...
DATA_ROOT = os.path.join(MASTERS, "data")          # data/{real,synth}/<scene>/
RESULTS_ROOT = os.path.join(HERE, "results")       # methods/3dtk_hough/results/<scene>/
OUT_PDF = os.path.join(MASTERS, "barrel_detection_results.pdf")
...
def page_for_dir(pdf, dirpath, name):
    ...
    if pts is None or pts.shape[0] == 0:
        ...
        return False
    cyl_path, src = find_cylinder_file(name)
...
def main():
    ...
    ordered = scenes("real") + scenes("synth")
