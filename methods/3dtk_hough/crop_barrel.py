...
def find_barrel_clusters(pcd_full, args, verbose=True):
    """Proposer shared by every method: from a full-resolution Open3D cloud,
    return barrel-candidate clusters as a list of (cluster_id, full_res_xyz_m).

    Steps (the heart of the old `main`):
      1. voxel downsample for speed
      2. RANSAC-remove the dominant planes (wall, floor)
      3. DBSCAN cluster the remainder
      4. keep ALL clusters matching the barrel priors (multi-barrel safe)
      5. project each kept cluster back to full-res via KDTree radius search

    `args` carries the same knobs the CLI exposes (voxel, plane_thresh,
    remove_planes, eps, min_pts, target_width, width_tol, max_depth_ratio,
    min_height, cz_min, cz_max, label_crop_dist). Geometry stays in the metric
    fit; this only answers "a barrel is here".
    """
    log = print if verbose else (lambda *a, **k: None)

    label_crop_dist = args.label_crop_dist
    if label_crop_dist is None:
        label_crop_dist = 1.5 * args.voxel

    pcd = pcd_full.voxel_down_sample(args.voxel)
    log(f"after voxel({args.voxel}m): {len(pcd.points)} pts")
    ...
    for i in range(args.remove_planes):
        ...
        a, b, c, d = plane
        log(f"plane[{i}]: {a:+.2f}x {b:+.2f}y {c:+.2f}z {d:+.2f}=0  "
            f"({len(inliers)} inliers)")
        rest = rest.select_by_index(inliers, invert=True)

    pts = np.asarray(rest.points)
    log(f"after plane removal: {pts.shape[0]} pts")
    ...
    # full-res point within label_crop_dist of any cluster point.
    ...
    for cid, sub in kept:
        m = full_res_mask_from_cluster(sub, full_tree, len(full_xyz),
                                       label_crop_dist)
        log(f"  cluster {cid}: {sub.shape[0]} downsampled -> "
            f"{m.sum()} full-res pts")
        out.append((cid, full_xyz[m]))
    return out
...
def main():
    ...
    kept = find_barrel_clusters(pcd_full, args)
    if not kept:
        sys.exit("no cluster matched barrel priors; "
                 "relax --width-tol / --max-depth-ratio / --min-height / --cz-*")

    xyz_m = np.concatenate([sub for _, sub in kept], axis=0)
